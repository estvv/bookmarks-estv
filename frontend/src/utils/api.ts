...
export const bookmarksApi = {
  list: (params: BookmarkListParams = {}): Promise<Bookmark[]> =>
    request(`/bookmarks${buildQuery(params)}`),

  get: (id: number): Promise<Bookmark> => request(`/bookmarks/${id}`),
  ...
  create: (bookmark: {
    ...
    request('/bookmarks', { method: 'POST', body: JSON.stringify(bookmark) }),

  fetchMeta: (url: string): Promise<PageMeta> =>
    request('/bookmarks/fetch-meta', { method: 'POST', body: JSON.stringify({ url }) }),
  ...
  update: (id: number, updates: Partial<{
    ...
    request(`/bookmarks/${id}`, { method: 'PUT', body: JSON.stringify(updates) }),

  delete: (id: number): Promise<void> => request(`/bookmarks/${id}`, { method: 'DELETE' }),

  setTags: (id: number, tagIds: number[]): Promise<Bookmark> =>
    request(`/bookmarks/${id}/tags`, { method: 'POST', body: JSON.stringify({ tagIds }) }),

  share: (id: number): Promise<{ share_token: string }> =>
    request(`/bookmarks/${id}/share`, { method: 'POST' }),

  unshare: (id: number): Promise<void> =>
    request(`/bookmarks/${id}/share`, { method: 'DELETE' }),

  reorder: (bookmarkIds: number[]): Promise<void> =>
    request('/bookmarks/reorder', { method: 'POST', body: JSON.stringify({ bookmarkIds }) }),
};
...
export const tagsApi = {
  ...
  create: (tag: { name: string; color?: string }): Promise<Tag> =>
    request('/tags', { method: 'POST', body: JSON.stringify(tag) }),
  findOrCreate: (name: string, color?: string): Promise<Tag> =>
    request('/tags/find-or-create', { method: 'POST', body: JSON.stringify({ name, color }) }),
  update: (id: number, updates: { name?: string; color?: string }): Promise<Tag> =>
    request(`/tags/${id}`, { method: 'PUT', body: JSON.stringify(updates) }),
  delete: (id: number): Promise<void> => request(`/tags/${id}`, { method: 'DELETE' }),
};

export const ioApi = {
  exportJson: (): Promise<{ folders: Folder[]; tags: Tag[]; bookmarks: any[] }> =>
    request('/io/export?format=json'),
  ...
  import: (format: 'json' | 'netscape', data: string | object): Promise<{ imported: number; skipped: number }> =>
    request('/io/import', { method: 'POST', body: JSON.stringify({ format, data }) }),
};
